...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    for(int i = 0; i < C; i++){
        if (h < H_out && w < W_out){
            shared[thread_idx] = in_4d(b,i,h,w);
        }else{
            shared[thread_idx] = 0.0;
        }
        __syncthreads();
        ...
        if (h < H_out && w < W_out){
            for (int p = 0; p < K; p++) // loop over KxK filter
                for (int q = 0; q < K; q++)
                acc += in_4d(b, i, h * S + p, w * S + q) * mask_4d(m, i, p, q);
        }
        __syncthreads();
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    dim3 gridDim(M, Y, B);
    conv_forward_kernel<<<gridDim, blockDim, Block_w*Block_w*sizeof(float)>>>(device_output, device_input, device_mask, B, M, C, H, W, K, S);
