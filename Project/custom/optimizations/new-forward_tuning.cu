    ...
    */
    ...
    if (h < H_out && w < W_out){
        for(int c = 0; c < C; c++){
            ...
            //acc += in_4d(b,c,h* S+p,w* S+3) * mask_4d(m,c,p,3);
            // acc += in_4d(b,c,h* S+p,w* S+4) * mask_4d(m,c,0,4);
            // acc += in_4d(b,c,h* S+p,w* S+5) * mask_4d(m,c,0,5);
            }
            else{
                acc += in_4d(b,c,h* S+0,w* S+0) * mask_4d(m,c,0,0);
                acc += in_4d(b,c,h* S+0,w* S+1) * mask_4d(m,c,0,1);
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    // We pass in_4d pointers for you to initialize the relevant device pointers,
